...
# 74 "overlayfs.c" 3 4
               stderr
# 74 "overlayfs.c"
               , "huse-overlayfs:" "\"mkdirs2\"" " fail: code=%d," "\"path=%s\"" "\n", errno, path);
                goto err;
            }
        }
        if (!*current)
...
# 102 "overlayfs.c" 3 4
               stderr
# 102 "overlayfs.c"
               , "huse-overlayfs:" "\"sendfile\"" " fail: code=%d," "\"ofd=%d, ifd=%d\"" "\n", errno, ofd, ifd);
            };
        }
        if (ifd != -1)
        {
...
static int (*original_openat)(int __fd, const char *__path, int __oflag, ...); void __attribute__((constructor)) init_openat(void) { original_openat = dlsym((void *)-1, "openat"); } int openat(int __fd, const char *__path, int __oflag, ...)
{
    int ret = 0;
    va_list args;
...
# 123 "overlayfs.c"
                          ;
    mode_t mode = (__oflag & 020000000 | 00200000 | 0100) ? 
# 124 "overlayfs.c" 3 4
                                       __builtin_va_arg(
# 124 "overlayfs.c"
                                       args
...
# 128 "overlayfs.c"
                                            )
    {
        for (char const *i = lower, *j = __path; *j; ++i, ++j)
        {
            if (*i == 0)
            {
                ...
                int result = original_openat(__fd, newPath, __oflag, mode);
                free(newPath);
                return result;
            }
            if (*i != *j)
    ...
    int result = original_openat(__fd, __path, __oflag, mode);
    return result;
}
