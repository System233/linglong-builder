...
PATCH_APP_NONSTD_PATH="s#/opt/apps/${LINGLONG_RAW_ID}/#${PREFIX}#g"
...
while read LINE; do
    ...
    if [[ $ICON == /* ]]; then
        REBASE_ICON=$(echo $ICON | sed -E -e "$PATCH_APP_PATH" -e "$PATCH_APP_NONSTD_PATH" -e "$PATCH_USR_PATH" -e "$PATCH_ENTRIES_PATH" | perl -pe "s#/opt/(?!apps)#$PREFIX/opt/#g")
